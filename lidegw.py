...
class Lide:
	...
	def listRooms (self):
		...
		for i in range(1, 8):
			...
			s = r.read().decode("iso-8859-2").encode(self.kernel.charset)
	...
	def updOPs(self, room):
		...
		r_data = r.read()
		half = re.findall("<a href=\"http://profil.lide.cz/(.+?)/profil/\" target=\"_blank\">.+?</a>", r_data)
                OPs = re.findall("[^a] href=\"http://profil.lide.cz/(.+?)/profil/\" target=\"_blank\">.+?</a>,", r_data)
